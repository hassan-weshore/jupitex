(function($) {
	$(function() {
		...
		function get_portfolio_sorted_items($portfolio, $activeItems) {
			if (!$('.portfolio-sorting a.sorting-switcher', $portfolio).length && !$('.portfolio-sorting-select', $portfolio).length) {
				return $activeItems;
			}
		...
		function get_portfolio_sorting_data($portfolio) {
			...
			if ($('.portfolio-sorting', $portfolio).length > 0) {
				...
				sorting = {
					sortBy: $('.portfolio-sorting-select li.current', $portfolio).data('orderby'),
					sortAscending: $('.portfolio-sorting-select li.current', $portfolio).data('order') == 'asc'
				};
			}
			return sorting;
		}
		...
		function init_products_new_sorting($portfolio) {
			...
			$('.portfolio-sorting-select li', $portfolio).on('click', function(e) {
				...
				$('.portfolio-sorting-select div.current', $portfolio).html($(this).html());
		...
		function initPortfolioGrid() {
			...
			portfolio_images_loaded($set, '.image-inner img', function() {
				...
				if (sortOptions && $('.portfolio-load-more', $portfolio).size() == 0 && !$portfolio.hasClass('portfolio-pagination-scroll')) {
		...
		$(document).ready(function() {
			$('body:not(.elementor-editor-active) .portfolio-grid:not(.extended-products-grid)').initPortfoliosGrid();
		});

		setTimeout(function () {
			if ($('body:not(.elementor-editor-active) .preloader + .portfolio-preloader-wrapper').length) {
				$('.portfolio-grid:not(.extended-products-grid)').initPortfoliosGrid();
			}
		}, 2000);
